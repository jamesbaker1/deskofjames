...
var NINE_DOTS_PUZZLE_DEFAULT_CONFIG = {
    show_hint_button_after_round: 100,
    text_button_hint: "Hint",
    text_hint: "Think outside the box.",
    colour_background: "rgb(255, 255, 255)",
    colour_neutral: "rgb(180, 180, 180)",
    colour_success: "rgb(0, 0, 0)",
    colour_fail: "rgb(220, 80, 80)",
    colour_face: "rgb(255, 255, 255)",
    colour_text: "rgb(100, 100, 100)",
    colour_text_background_dark: "rgb(240, 240, 240)",
    colour_text_background_light: "rgb(250, 250, 250)",
    hints: [
        "there are no edges",
        "the grid is a suggestion",
        "extend past the dots"
    ]
};
...
(function(canvasId, defaultConfig) {
    ...
    NineDotsPuzzle.prototype = {
        ...
        updateCurrentHint : function() {
            var hints = this.context.get("hints") || [];
            var hintIndex = Math.min(this.currentRound - 1, hints.length - 1);
            this.currentHint = (hintIndex >= 0 && hints[hintIndex]) ? hints[hintIndex] : null;
        },
        
        drawHintText : function() {
            if (this.currentHint) {
